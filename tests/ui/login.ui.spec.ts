...
test.describe('ui/login', () => {
  test('allows an operator to reach the payments workspace', { tag: ['@smoke', '@critical'] }, async ({ loginPage, paymentsPage, operatorCredentials }) => {
  ...
  test('shows a concise inline error for invalid credentials', { tag: ['@regression'] }, async ({ loginPage }) => {
    await loginPage.goto();
